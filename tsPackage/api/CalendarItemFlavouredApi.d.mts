...
export interface CalendarItemFlavouredApi<E extends CalendarItem> {
    shareWith(delegateId: string, calendarItem: E, options?: {
        options?: CalendarItemShareOptions | undefined;
    }): Promise<E>;
    shareWithMany(calendarItem: E, delegates: {
        [key: string]: CalendarItemShareOptions;
    }): Promise<E>;
    findCalendarItemsByHcPartyPatient(hcPartyId: string, patient: Patient, options?: {
        startDate?: number | undefined;
        endDate?: number | undefined;
        descending?: boolean | undefined;
    }): Promise<PaginatedListIterator<E>>;
    linkToPatient(calendarItem: CalendarItem, patient: Patient, shareLinkWithDelegates: Array<string>): Promise<E>;
    ...
    getCalendarItemsByPeriodAndHcPartyId(startDate: number, endDate: number, hcPartyId: string): Promise<Array<E>>;
    getCalendarsByPeriodAndAgendaId(startDate: number, endDate: number, agendaId: string): Promise<Array<E>>;
    findCalendarItemsByRecurrenceId(recurrenceId: string, startKey: string | undefined, startDocumentId: string | undefined, limit: number): Promise<PaginatedList<E>>;
}
